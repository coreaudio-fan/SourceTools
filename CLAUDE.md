...
Reflow semantics follow a **unit model read entirely from tabs and newlines** (`Framework/ReflowParagraphs.swift`): blank and leader-only lines separate paragraphs; within a paragraph, consecutive plain lines at the same text-start column form a prose unit that joins and refills, while a line containing an interior tab is *columnated* — everything through its last interior tab stays verbatim and only the text after it flows, with continuations padded (pure tabs; a hanging column is always a tab stop) back to that column. A plain line starting exactly at a columnated unit's hanging column is absorbed as overflow from a previous wrap, so editing wrapped text round-trips, and reflowing unchanged output is a no-op. Tab-aligned DocC `- Parameters:` blocks therefore keep their structure; the worked examples live in `Tests/ReflowTests.swift` as `alignedParameterListsReflowWithoutJoining` and `absorbedOverflowRejoinsItsColumnatedLine`.
